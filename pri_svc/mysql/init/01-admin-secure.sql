...
CREATE USER IF NOT EXISTS 'gate_admin'@'192.168.%' IDENTIFIED BY 'gate_pass';
GRANT ALL PRIVILEGES ON *.* TO 'gate_admin'@'192.168.%';
-- 필요 시 특정 DB로만 축소: GRANT ALL ON appdb.* TO 'dbgate_admin'@'172.22.%';

-- (선택) 네트워크 내부라도 전송 암호화를 강제하고 싶다면 아래 사용(서버 TLS 설정 필요)
-- ALTER USER 'dbgate_admin'@'172.22.%' REQUIRE SSL;
